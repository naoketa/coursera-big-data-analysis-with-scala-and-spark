...
object TimeUsage extends TimeUsageInterface {
  ...
  def dfSchema(columnNames: List[String]): StructType = {
    StructType(
      StructField(columnNames.head, StringType, false) :: columnNames.tail.map((columnName: String) => StructField(columnName, DoubleType, false))
    )
  }
  ...
  def classifiedColumns(columnNames: List[String]): (List[Column], List[Column], List[Column]) = {
    ...
    (
      columnNames
        .filter(column => primaryNeeds.exists(c => column.startsWith(c))).map(new ColumnName(_)),
      columnNames
        .filter(column => workingActivities.exists(c => column.startsWith(c))).map(new ColumnName(_)),
      columnNames
        .filter(column => otherActivities
          .exists(c => column.startsWith(c)
            && !column.startsWith("t1801")
            && !column.startsWith("t1803")
